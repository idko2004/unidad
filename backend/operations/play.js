	...
	*/
	...
	if(cardProperties.type === cards.cardTypes.moreSpecial)
	{
		...
		switch(cardProperties.value)
		{
			...
			case 'INTERCHANGE':
				playInterchangeCard(dataObject, ws, room, messages);
				break;
		}
	}
...
function playInterchangeCard(dataObject, ws, room, messages)
{
	if([dataObject, ws, room].includes(undefined))
	{
		console.log(colors.red('play.playPlusOneCard: alguno de los parámetros es undefined'));
	...
	{
		console.log('La sala tiene más de 2 personas');
		previous = game.utils.whoWasBefore(roomID);
		next = game.utils.whosNext(roomID);
	}
	else
	{
		console.log('En la sala solo son 2');
		previous = 0;
		next = 1;
	}

	const previousName = room.order[previous];
	const nextName = room.order[next];

	console.log(`Intercambio de cartas: previous: ${previous} ${previousName}, next: ${next} ${nextName}`);

	if([previousName, nextName].includes(undefined))
	{
		ws.send(JSON.stringify(
		{
			operation: 'errorPlaying',
			error: 'interchangePlayerError'
		}));
		console.log(`play.playInterchangeCard: No se pudo obtener quien iba antes y después: Antes: ${previous} ${previousName}, Después: ${next} ${nextName}`);
		return;
	}

	const previousDeck = room.players[previousName].deck;
	const nextDeck = room.players[nextName].deck;

	let previousNumber;
	let nextNumber;

	if(previousDeck.length > 1) previousNumber = random.range(0, previousDeck.length);
	else previousNumber = 0;

	if(nextDeck.length > 1) nextNumber = random.range(0, nextDeck.length);
	else nextNumber = 0;

	const previousCard = previousDeck[previousNumber];
	const nextCard = nextDeck[nextNumber];

	console.log(`previousCard: ${previousNumber} ${previousCard}, nextCard: ${nextNumber} ${nextCard}`);

	if([previousCard, nextCard].includes(undefined))
	{
		ws.send(JSON.stringify(
		{
			operation: 'errorPlaying',
			error: 'interchangeCardError'
		}));
		console.log(`play.playInterchangeCard: No se pudo obtener quien iba antes y después: Antes: ${previous} ${previousName}, Después: ${next} ${nextName}`);
		return;
	}

	room.players[previousName].deck = cards.deleteFromDeck(previousCard, previousDeck);
	room.players[nextName].deck = cards.deleteFromDeck(nextCard, nextDeck);

	room.players[previousName].deck.push(nextCard);
	room.players[nextName].deck.push(previousCard);

	messages.push(msg.getMessage(msg.msgValues.interchangeCards,
	{
		player1: previousName,
		player2: nextName
	}));
